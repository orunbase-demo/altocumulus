...
Your backend holds one Altocumulus API key and **zero GitHub credentials**.
...
const altocumulus = new Altocumulus({
  baseUrl: process.env.ALTOCUMULUS_API_URL!,
  token: process.env.ALTOCUMULUS_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await altocumulus.integrations.issueGithubToken(ORG_ID, {
